...
interface RegistrationProps {
  onShowAuthorization: () => void;
}

export function Registration({ onShowAuthorization }: RegistrationProps): JSX.Element {
  const {
    info,
    alert,
    page,
      ...
      <IonCard className="l-card-1 pb-1">
        ...
        <div className="ml-2 mt-1 mr-2">
          ...
          <div className="borders-wp pl-1 ml-1 mt-1 bg-2">
            <IonInput
              ...
              onIonChange={(e) => {
                setInfo({ ...info, КонтактныйТелефон: e.target.value as string });
              }}
            />
          </div>
          <div className="borders-wp mt-1 pl-1 ml-1 bg-2">
            <IonInput
              ...
              placeholder="e-mail"
              onIonChange={(e) => {
                setInfo({ ...info, элПочта: e.target.value as string });
              }}
            />
          </div>
        </div>
        <div className="ml-2 mr-1 pt-1">
          <IonCheckbox
            ...
            checked={info.Согласие}
            onIonChange={(e) => {
              setInfo({ ...info, Согласие: e.detail.checked });
            }}
          >
      ...
      <IonCard className="l-card-1 pb-1">
            ...
            <Files
              info={{
                ...
                Описание:
                  info?.ИНН?.length > 10
                    ? "Копия паспорта"
                    : "Доверенность либо приказ (письменное уполномочие права на официальное представление от имени организации)",
                Обязателен: false,
                Файлы: info.Файлы["Устав"],
              }}
            />
          ) : index === 1 ? (
            <Files
              info={{
                ...
                Описание:
                  info?.ИНН?.length > 10
                    ? "Карточка ИП (юр. адрес, конт.тел. эл. почта, р/с, ОКВЭД)"
                    : "Карточка ЮЛ (юр. адрес, конт.тел. эл. почта, р/с, ОКВЭД)",
                Обязателен: false,
                Файлы: info.Файлы["Карточка"],
              }}
            />
          ) : (
            <></>
        ...
        <div className="flex fl-space mr-1 ml-1 mt-1">
          ...
          <div>
            ...
            >
              {page === 1 ? (index === 1 ? "Отправить" : "Далее") : "Далее"}
            </IonButton>
          </div>
        </div>
      </IonCard>
      ...
      <div className={isPlatform("ios") ? "l-content mt-3" : "l-content"}>
        <div className="p-page mr-auto ml-auto">
          <IonCard className="l-card-1 mt-2 pb-1">
            <div className="flex ml-1 mt-1 fs-bold">
              У вас уже есть аккаунт?
              <span
                className="t-underline ml-1 cl-yellow"
                onClick={() => {
                  onShowAuthorization();
                }}
              >
